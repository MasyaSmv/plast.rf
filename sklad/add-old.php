...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(isset($_POST['c_id'])){
			// пропарсить файл+, проверить компанию+, удалить старые данные+, заменить новыми+.
		$error=0;
...
";				
			...
			$msg.="<p style='color:red;font-size:110%;'>Результат обработки: прочитано из файла строк: $k</p>";
			unlink($tmpfname);
			$msg.="<p style='color:red;font-size:110%;'><a href=/admin2/sklad/showt.php>Посмотреть, как добавилось (просмотр позиций)</a></p>";	
		}
	}
}
...
<tr class=color4>
<td>1)</td><td width=150 valign=top>Пользователь, логин</td><td><?=$link?></td>
</tr>
<tr class=color4>
<td>2)</td><td width=150 valign=top>ID пользователя<br>(опционально)</td><td><input type=text size=6 id="company2" name=c_id value='<?=$c_idv?>'></td>
</tr>
<tr class=color4><td>3)</td><td>Xls файл</td><td><input type=file name="ufile"></td></tr>
<tr class=color4><td>4)</td><td>Уведомление польз. на email</td><td><input type=checkbox name="uved" checked></td></tr>
...
<p>
При добавлении нового склада все имеющиеся в базе позиции этого пользователя будут удалены.
</p>
<hr>
