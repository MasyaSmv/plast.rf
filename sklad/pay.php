...
$site_nav[]=array("name"=>"Склад - Платные клиенты (тариф Безлимитный)</a>","url"=>"/$document_admin/sklad/pay.php");
...
if (!isset($_GET['page']) || ($_GET['page'] <= 0)) 
	...
	$count = ssql("SELECT count(*) from company_usl_list where tarif_id=18 order by id");
...
//	$q="select distinct comp_id as cid, count(*) as kol, date_cr from sklad_tovar group by comp_id order by date_cr  LIMIT ".$start_pos.", ".$perpage;
	...
	$q="select cul.company_id as cid, DATE_FORMAT( cul.date_begin , '%d.%m.%Y' ),DATE_FORMAT( cul.date_end , '%d.%m.%Y' ), c.title from company_usl_list cul,company c where  cul.tarif_id=18 and c.id=cul.company_id order by cul.date_end desc  LIMIT ".$start_pos.", ".$perpage;
	$r=sql($q);
...
<td>Организация</td><td>Дата начала</td><td>Дата окончания</td>
...
<td><a href=/admin2/or/?a=edit&id='.$cid.'&page='.$_GET['page'].'&c='.$cid.'>'.$title.'</a></td><td>'.$date_b.' </td><td>'.$date_e.'</td></tr>';
