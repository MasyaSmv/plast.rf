...
//	$q="select distinct comp_id as cid, count(*) as kol, date_cr from sklad_tovar group by comp_id order by date_cr  LIMIT ".$start_pos.", ".$perpage;
	...
	$q="select cul.company_id as cid, DATE_FORMAT( cul.date_begin , '%d.%m.%Y' ),DATE_FORMAT( cul.date_end , '%d.%m.%Y' ), c.title from company_usl_list cul,company c where  cul.tarif_id=19 and c.id=cul.company_id order by cul.date_end desc  LIMIT ".$start_pos.", ".$perpage;
	$r=sql($q);
